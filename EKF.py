...
class EKF:
    ...
    def setControlCovariance(self, cov):
        self.sigmaU = cov
    ...
    def computeMeasurementJacobian(self, range, bearing, robotState):
        ...
        jacobian = np.array([[xDist/np.sqrt(denominator), yDist/np.sqrt(denominator)],\
                            [-yDist/denominator, xDist/denominator ]])
    ...
    def correct(self, range, bearing, robotState):
        ...
        self.stateCovariance = (np.identity(3) - K @ H) @ self.stateCovariance

        # Note: Notation on z is inconsistent on p. 450
        return np.linalg.det(2*np.pi*Q)**-.5 * np.exp(-.5(zt-z).T @ np.linal.inv(Q) @ (zt-z))
